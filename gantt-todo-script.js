...
function renderPendingTasks(pending) {
    ...
    for (var i = 0; i < page.length; i++) {
        ...
        html += "<tr><td class=\"col-index\">" + idx + "</td><td class=\"col-check\"><label class=\"gantt-todo-check-wrap\"><input type=\"checkbox\" data-action=\"complete\" data-note-id=\"" + escHtml(t.noteId) + "\" data-task-idx=\"" + t.taskIndexInNote + "\"><span class=\"gantt-todo-checkmark\"></span></label></td><td class=\"col-content\"><span class=\"gantt-todo-task-text\" data-note-id=\"" + escHtml(t.noteId) + "\" data-task-idx=\"" + t.taskIndexInNote + "\">" + escHtml(t.text) + rB + "</span>" + (t.notePath ? "<div class=\"gantt-todo-note-path\">" + escHtml(t.notePath) + "</div>" : "") + "</td><td class=\"col-note\" data-note-id=\"" + escHtml(t.noteId) + "\" title=\"" + escHtml(t.noteTitle) + "\">" + escHtml(t.noteTitle) + "</td><td class=\"col-priority\"><span class=\"gantt-todo-priority " + pC + "\">" + pL + "</span></td><td class=\"col-date\">" + fmtDate(t.startDate) + "</td><td class=\"col-date\">" + fmtDate(t.endDate) + "</td></tr>";
    }
    body.innerHTML = html;

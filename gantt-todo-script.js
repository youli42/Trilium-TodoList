...
function readSettings() {
  try { var raw = localStorage.getItem(STORAGE_KEY); return raw ? Object.assign({}, DEFAULT_SETTINGS, JSON.parse(raw)) : Object.assign({}, DEFAULT_SETTINGS); }
...
async function loadTasks() {
  ...
  var promise = (async function() {
    try {
      ...
      var backend = (typeof runBackendAction === "function") ? runBackendAction : null;
      if (!backend) throw new Error("runBackendAction not available");
      var result = await backend("fetchTasks", { rootNoteIds: scope });
      if (!result || result.emptyScope) { taskData = []; return []; }
      taskData = (result.tasks) || [];
      return taskData;
    } catch (err) { console.error("[GanttTodo] loadTasks failed:", err); throw err; }
...
async function doCompleteTask(task) {
  var backend = (typeof runBackendAction === "function") ? runBackendAction : null;
  if (!backend) throw new Error("Backend unavailable");
  return backend("completeTask", { task: task, settings: readSettings() });
}
async function doUncompleteTask(task) {
  var backend = (typeof runBackendAction === "function") ? runBackendAction : null;
  if (!backend) throw new Error("Backend unavailable");
  return backend("uncompleteTask", { task: task });
}

function showEmptyScopeMessage() {
  var exampleNoteId = "gXEI9kcMRw7F";
  ...
  dom.ganttContainer.innerHTML = "<div class=\"gantt-todo-empty\">请在设置页面中配置任务收集范围（笔记ID），例如: " + escHtml(exampleNoteId) + "</div>";
}
...
function switchTab(tab) {
  ...
  else if (tab === "list") { renderTaskList(); }
...
function refreshStats() {
  if (currentTab === "gantt" && taskData) {
    ...
    var done = taskData.filter(function(t) { return t.done; }).length;
...
function renderGantt() {
  ...
  if (!taskData) return;
  var container = dom.ganttContainer;
  var statsEl = dom.ganttStats;
  var datedTasks = taskData.filter(function(t) { return t.startDate || t.endDate; });
  ...
  var ganttTasks = datedTasks.map(function(t) {
    ...
    var sFull = start + "T00:00:00";
    var eFull = end + "T23:59:00";
    return { id: t.noteId + "-" + t.taskIndexInNote, name: t.text || "(empty)", start: sFull, end: eFull, progress: t.done ? 100 : 0, dependencies: "", custom_class: t.done ? "gantt-task-complete" : "" };
  }).filter(Boolean);
  ...
  statsEl.textContent = "共 " + taskData.length + " 个，甘特图显示 " + ganttTasks.length + " 个";
  container.innerHTML = "";
...
function renderTaskList() {
  ...
  var maxPP = Math.max(1, Math.ceil(pending.length / pageSize));
  var maxCP = Math.max(1, Math.ceil(completed.length / pageSize));
  pendingPage = clamp(pendingPage, 1, maxPP);
  completedPage = clamp(completedPage, 1, maxCP);
  ...
  var pVisible = pending.length > 0;
  var cVisible = completed.length > 0;
...
function renderPendingTasks(pending) {
  ...
  var start = (pendingPage - 1) * pageSize;
  var page = pending.slice(start, start + pageSize);
  if (page.length === 0) { body.innerHTML = "<tr><td colspan=\"6\" class=\"gantt-todo-empty\">暂无待办任务</td></tr>"; meta.textContent = ""; return; }
  var html = "";
  for (var i = 0; i < page.length; i++) {
    var t = page[i], idx = start + i + 1, pC = priorityClass(t.priority), pL = priorityLabel(t.priority), rB = t.repeat ? "<span class=\"gantt-todo-repeat-badge\">&#x1F504;</span>" : "";
    html += "<tr><td class=\"col-index\">" + idx + "</td><td class=\"col-check\"><label class=\"gantt-todo-check-wrap\"><input type=\"checkbox\" data-action=\"complete\" data-note-id=\"" + escHtml(t.noteId) + "\" data-task-idx=\"" + t.taskIndexInNote + "\"><span class=\"gantt-todo-checkmark\"></span></label></td><td class=\"col-content\"><span class=\"gantt-todo-task-text\" data-note-id=\"" + escHtml(t.noteId) + "\" data-task-idx=\"" + t.taskIndexInNote + "\">" + escHtml(t.text) + rB + "</span>" + (t.notePath ? "<div class=\"gantt-todo-note-path\">" + escHtml(t.notePath) + "</div>" : "") + "</td><td class=\"col-priority\"><span class=\"gantt-todo-priority " + pC + "\">" + pL + "</span></td><td class=\"col-date\">" + fmtDate(t.startDate) + "</td><td class=\"col-date\">" + fmtDate(t.endDate) + "</td></tr>";
  }
  body.innerHTML = html;
...
function renderCompletedTasks(completed) {
  ...
  var start = (completedPage - 1) * pageSize;
  var page = completed.slice(start, start + pageSize);
...
async function handleCheckboxChange(cb) {
  ...
  try {
    if (action === "complete") { await doCompleteTask(task); task.done = true; }
    else { await doUncompleteTask(task); task.done = false; }
    if (currentTab === "list") renderTaskList();
    refreshStats();
  } catch (err) { console.error("[GanttTodo] Toggle failed:", err); cb.checked = !cb.checked; }
}

function handleTaskTextClick(el) {
  var nid = el.dataset.noteId;
  if (nid && api && typeof api.activateNote === "function") api.activateNote(nid);
}

function doSearch() {
  var q = (dom.searchInput.value || "").trim();
  if (q === searchQuery) return;
  searchQuery = q; pendingPage = 1; completedPage = 1; renderTaskList();
}
